...
class ArticleController extends Controller {
    ...
    public function uploadBackground() {
        ...
        if(isset($_FILES['image'])){
            ...
            // Đặt tên file theo quy tắc article{id}.ext
            $final_name = "article" . $id . $ext;

            $folder = $target_dir . $final_name;

            // Nếu đã tồn tại thì ghi đè
    ...
    public function getContent() {
        ...
        $id = intval($_GET['id']); 
        $article = new Article();
        $article->setIdArticle($id); // tự động load dữ liệu từ DB

        $content = $article->getContent(); // lấy content đã load
